    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto space-y-8 pb-12">
      <header className="flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="text-xl font-semibold tracking-tight text-text-primary">{content.title}</h1>
          <p className="text-text-muted text-xs font-medium mt-1">Published: {content.publishedAt}</p>
        </div>
      </header>

      {/* Level 5: Content Performance */}
      <section>
        <h2 className="text-sm font-semibold uppercase tracking-widest text-text-primary mb-4">Content Performance</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Views', value: content.metrics.views.toLocaleString(), icon: <BarChart2 className="w-4 h-4 text-text-muted" /> },
            { label: 'Likes', value: content.metrics.likes?.toLocaleString() || '0', icon: <Heart className="w-4 h-4 text-text-muted" /> },
            { label: 'Comments', value: content.metrics.comments?.toLocaleString() || '0', icon: <MessageCircle className="w-4 h-4 text-text-muted" /> },
            { label: 'Shares', value: content.metrics.shares?.toLocaleString() || '0', icon: <Share2 className="w-4 h-4 text-text-muted" /> },
          ].map(m => (
            <div key={m.label} className="p-5 rounded-xl border border-border bg-bg-elevated">
      ...
      {/* Detailed Metrics */}
      <section className="bg-bg-elevated border border-border rounded-2xl p-6">
        <h3 className="text-sm font-semibold uppercase tracking-widest text-text-primary mb-6">Deep Dive Metrics</h3>
        
        <div className="space-y-4">
          <div className="flex justify-between items-center py-3 border-b border-border-light">
            <span className="text-sm font-medium text-text-muted">Reach</span>
            <span className="text-sm font-bold text-text-primary">{content.metrics.reach?.toLocaleString() || 'N/A'}</span>
          </div>
          <div className="flex justify-between items-center py-3 border-b border-border-light">
            <span className="text-sm font-medium text-text-muted">Saves</span>
            <span className="text-sm font-bold text-text-primary">{content.metrics.saves?.toLocaleString() || '0'}</span>
          </div>
          <div className="flex justify-between items-center py-3">
            <span className="text-sm font-medium text-text-muted">Total Engagements</span>
            <span className="text-sm font-bold text-text-primary">{content.metrics.engagements.toLocaleString()}</span>
          </div>
        </div>
      </section>
