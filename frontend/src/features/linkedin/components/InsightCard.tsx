...
export const InsightCard: React.FC<InsightCardProps> = ({ insight }) => {
  ...
  const getStyles = () => {
    switch (insight.type) {
      ...
      default: return 'border-border-light bg-bg-elevated hover:bg-bg-sunken';
    }
  };
  ...
  return (
    <div className={`p-6 rounded-2xl border transition-all cursor-pointer ${getStyles()}`}>
      ...
      <div className="mt-4 text-[11px] text-text-muted leading-relaxed">
        <span className="font-bold text-text-secondary">Confidence {insight.confidence}%:</span> {insight.supportingMetrics.join(' | ')}
      </div>
